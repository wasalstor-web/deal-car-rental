...
const ContactForm = ({ user, className }: ContactFormProps) => {
  ...
  return (
    <Paper className={`${className ? `${className} ` : ''}contact-form`} elevation={10}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} dir={language === 'ar' ? 'rtl' : 'ltr'} lang={language}>
        {!isAuthenticated && (
        ...
        <div className="buttons">
          <Button type="submit" variant="contained" className="btn-primary btn-margin-bottom btn" aria-label={strings.SEND} disabled={isSubmitting}>
            {
              isSubmitting
