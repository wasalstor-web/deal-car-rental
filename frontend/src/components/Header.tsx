      ...
      <div style={classes.grow} className="header">
        <AppBar position="relative" sx={{ bgcolor: '#fff', boxShadow: 'none', borderBottom: '1px solid #ddd' }}>
          <Toolbar className="toolbar">
            {isLoaded && (
              <>
                <IconButton edge="start" sx={classes.menuButton} aria-label={strings.OPEN_SIDE_MENU} onClick={handleSideMenuOpen}>
                  <MenuIcon />
                </IconButton>

                <Button onClick={() => navigate('/')} className="logo" aria-label={strings.HOME_LINK}>{env.WEBSITE_NAME}</Button>
            ...
            <div className="header-desktop">
              {isLoaded && (
                <Button variant="contained" onClick={handleCurrencyMenuOpen} disableElevation className="btn bold" aria-label={strings.SELECT_CURRENCY}>
                  {PaymentService.getCurrency()}
                </Button>
              )}
              {isLoaded && (
                <Button variant="contained" onClick={handleLangMenuOpen} disableElevation className="btn" aria-label={strings.SELECT_LANGUAGE}>
                  <div className="language">
              ...
              {!hideSignin && !isSignedIn && isLoaded && (
                <Button variant="contained" size="medium" startIcon={<SignUpIcon />} onClick={() => navigate('/sign-up')} disableElevation className="btn btn-auth" aria-label={suStrings.SIGN_UP}>
                  <span className="btn-auth-txt">{suStrings.SIGN_UP}</span>
                </Button>
              )}
              {!hideSignin && !isSignedIn && isLoaded && (
                <Button variant="contained" size="medium" startIcon={<LoginIcon />} onClick={() => navigate('/sign-in')} disableElevation className="btn btn-auth" aria-label={strings.SIGN_IN}>
                  <span className="btn-auth-txt">{strings.SIGN_IN}</span>
                </Button>
              )}
              {isSignedIn && (
                <IconButton aria-label={strings.NOTIFICATIONS} onClick={handleNotificationsClick} className="btn">
              ...
              {isSignedIn && (
                <IconButton edge="end" aria-label={strings.ACCOUNT_MENU} aria-controls={menuId} aria-haspopup="true" onClick={handleAccountMenuOpen} className="btn">
                  <Avatar loggedUser={currentUser} user={currentUser} size="small" readonly />
                </IconButton>
              )}
            </div>
            <div className="header-mobile">
              <Button variant="contained" onClick={handleCurrencyMenuOpen} disableElevation fullWidth className="btn bold" aria-label={strings.SELECT_CURRENCY}>
                {PaymentService.getCurrency()}
              </Button>
              <Button variant="contained" onClick={handleLangMenuOpen} disableElevation fullWidth className="btn" aria-label={strings.SELECT_LANGUAGE}>
                <div className="language">
                  <CircleFlag countryCode={lang?.countryCode as string} height={flagHeight} className="flag" title={lang?.label} />
              ...
              {isSignedIn && (
                <IconButton aria-label={strings.MORE_OPTIONS} aria-controls={mobileMenuId} aria-haspopup="true" onClick={handleMobileMenuOpen} className="btn">
                  <MoreIcon />
                </IconButton>
              )}
            </div>
