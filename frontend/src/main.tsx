...
import { frFR as corefrFR, enUS as coreenUS, esES as coresES, arSA as coreArSA } from '@mui/material/locale'
import { frFR, enUS, esES } from '@mui/x-date-pickers/locales'
import { frFR as dataGridfrFR, enUS as dataGridenUS, esES as dataGridesEs, arSD as dataGridArSD } from '@mui/x-data-grid/locales'
...
/**
 * Apply locale to every LocalizedStrings instance (must run on bootstrap whenever language is known).
 */
const syncAllLocalizedStrings = (_lang: string) => {
...
if (lang) {
  ...
  try {
    ...
    syncAllLocalizedStrings(language)
  } catch (err) {
  ...
  if (storedLang) {
    syncAllLocalizedStrings(storedLang)
  }

  if (env.SET_LANGUAGE_FROM_IP && !storedLang) {
    const country = await IpInfoService.getCountryCode()
    const arabCountries = ['SA', 'AE', 'EG', 'KW', 'QA', 'OM', 'BH', 'JO', 'IQ', 'LB', 'PS', 'YE', 'DZ', 'TN', 'LY', 'SD', 'MR']

    if (arabCountries.includes(country)) {
      UserService.setLanguage('ar')
      syncAllLocalizedStrings('ar')
    } else if (['FR', 'MA'].includes(country)) {
      UserService.setLanguage('fr')
      syncAllLocalizedStrings('fr')
    } else if (['US', 'GB', 'AU'].includes(country)) {
      UserService.setLanguage('en')
      syncAllLocalizedStrings('en')
    } else {
      UserService.setLanguage(env.DEFAULT_LANGUAGE)
      syncAllLocalizedStrings(env.DEFAULT_LANGUAGE)
    }
  }
}
...
const theme = createTheme(
  {
    ...
    typography: {
      fontFamily: isAr
        ? '"Segoe UI",Tahoma,"Noto Sans Arabic","Dubai","Arial",sans-serif,"Apple Color Emoji","Segoe UI Emoji"'
        : [
            '-apple-system',
            'BlinkMacSystemFont',
            '"Segoe UI"',
  ...
  isFr ? dataGridfrFR : isEs ? dataGridesEs : isAr ? dataGridArSD : dataGridenUS,
  isFr ? corefrFR : isEs ? coresES : isAr ? coreArSA : coreenUS,
)
...
ReactDOM.createRoot(document.getElementById('root')!).render(isAr ? <CacheProvider value={cacheRtl}>{appTree}</CacheProvider> : appTree)
