...
describe('POST /api/checkout', () => {
  it('should checkout', async () => {
    ...
    const stripeReady = /^sk_(test|live)_/.test(env.STRIPE_SECRET_KEY)

    // test failure (stripe payment failed) + success path — requires a real Stripe secret in .env
    if (stripeReady) {
      ...
      // No real Stripe key: keep payLater flow; next block still expects 400 when payLater is false without ids
      payload.payLater = false
    }
