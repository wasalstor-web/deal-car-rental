...
/** French typography uses a space before ':'; other locales use a plain colon. */
const intlColon = (language: string) => (bookcarsHelper.isFrench(language) ? ' : ' : ': ')
...
export const getCancellation = async (cancellation: number, language: string, priceChangeRate: number) => {
  ...
  if (cancellation === -1) {
    return `${i18n.t('CANCELLATION')}${intlColon(language)}${i18n.t('UNAVAILABLE')}`
  } if (cancellation === 0) {
    return `${i18n.t('CANCELLATION')}${intlColon(language)}${i18n.t('INCLUDED')}${fr ? 'e' : ''}`
  }
  let _cancellation = await StripeService.convertPrice(cancellation)
  _cancellation += _cancellation * (priceChangeRate / 100)
  return `${i18n.t('CANCELLATION')}${intlColon(language)}${bookcarsHelper.formatPrice(_cancellation, (await StripeService.getCurrencySymbol()), language)}`
}
...
export const getAmendments = async (amendments: number, language: string, priceChangeRate: number) => {
  ...
  if (amendments === -1) {
    return `${i18n.t('AMENDMENTS')}${intlColon(language)}${i18n.t('UNAVAILABLE')}${fr ? 's' : ''}`
  } if (amendments === 0) {
    return `${i18n.t('AMENDMENTS')}${intlColon(language)}${i18n.t('INCLUDED')}${fr ? 'es' : ''}`
  }
  let _amendments = await StripeService.convertPrice(amendments)
  _amendments += _amendments * (priceChangeRate / 100)
  return `${i18n.t('AMENDMENTS')}${intlColon(language)}${bookcarsHelper.formatPrice(_amendments, (await StripeService.getCurrencySymbol()), language)}`
}
...
export const getTheftProtection = async (theftProtection: number, language: string, priceChangeRate: number) => {
  ...
  if (theftProtection === -1) {
    return `${i18n.t('THEFT_PROTECTION')}${intlColon(language)}${i18n.t('UNAVAILABLE')}`
  } if (theftProtection === 0) {
    return `${i18n.t('THEFT_PROTECTION')}${intlColon(language)}${i18n.t('INCLUDED')}${fr ? 'e' : ''}`
  }
  ...
  return `${i18n.t('THEFT_PROTECTION')}${intlColon(language)}${bookcarsHelper.formatPrice(_theftProtection, (await StripeService.getCurrencySymbol()), language)}${i18n.t('DAILY')}`
}
...
export const getCollisionDamageWaiver = async (collisionDamageWaiver: number, language: string, priceChangeRate: number) => {
  ...
  if (collisionDamageWaiver === -1) {
    return `${i18n.t('COLLISION_DAMAGE_WAVER')}${intlColon(language)}${i18n.t('UNAVAILABLE')}`
  } if (collisionDamageWaiver === 0) {
    return `${i18n.t('COLLISION_DAMAGE_WAVER')}${intlColon(language)}${i18n.t('INCLUDED')}${fr ? 'e' : ''}`
  }
  ...
  return `${i18n.t('COLLISION_DAMAGE_WAVER')}${intlColon(language)}${bookcarsHelper.formatPrice(_collisionDamageWaiver, (await StripeService.getCurrencySymbol()), language)}${i18n.t('DAILY')}`
}
...
export const getFullInsurance = async (fullInsurance: number, language: string, priceChangeRate: number) => {
  ...
  if (fullInsurance === -1) {
    return `${i18n.t('FULL_INSURANCE')}${intlColon(language)}${i18n.t('UNAVAILABLE')}`
  } if (fullInsurance === 0) {
    return `${i18n.t('FULL_INSURANCE')}${intlColon(language)}${i18n.t('INCLUDED')}${fr ? 'e' : ''}`
  }
  let _fullInsurance = await StripeService.convertPrice(fullInsurance)
  _fullInsurance += _fullInsurance * (priceChangeRate / 100)
  return `${i18n.t('FULL_INSURANCE')}${intlColon(language)}${bookcarsHelper.formatPrice(_fullInsurance, (await StripeService.getCurrencySymbol()), language)}${i18n.t('DAILY')}`
}
...
export const getAdditionalDriver = async (additionalDriver: number, language: string, priceChangeRate: number) => {
  ...
  if (additionalDriver === -1) {
    return `${i18n.t('ADDITIONAL_DRIVER')}${intlColon(language)}${i18n.t('UNAVAILABLE')}`
  } if (additionalDriver === 0) {
    return `${i18n.t('ADDITIONAL_DRIVER')}${intlColon(language)}${i18n.t('INCLUDED')}`
  }
  ...
  return `${i18n.t('ADDITIONAL_DRIVER')}${intlColon(language)}${bookcarsHelper.formatPrice(_additionalDriver, (await StripeService.getCurrencySymbol()), language)}${i18n.t('DAILY')}`
}
...
export const getDays = (days: number) =>
  `${i18n.t('PRICE_DAYS_PART_1')} ${days} ${days > 1 ? i18n.t('PRICE_DAYS_PART_2_PLURAL') : i18n.t('PRICE_DAYS_PART_2')}`
...
export const getDaysShort = (days: number) =>
  `${days} ${days > 1 ? i18n.t('PRICE_DAYS_PART_2_PLURAL') : i18n.t('PRICE_DAYS_PART_2')}`
...
export const getDepositFilterValue = async (language: string, value: DepositFilterValue): Promise<string> => {
  ...
  const prefix = String(i18n.t('DEPOSIT_FILTER_LESS_THAN_PREFIX', { locale: language }))
  return `${prefix} ${isCurrencyRTL ? currency : ''}${depositFilterValue}${!isCurrencyRTL ? (` ${currency}`) : ''}`
}
