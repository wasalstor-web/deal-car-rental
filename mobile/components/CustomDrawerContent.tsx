...
import { setI18nLocaleOnly, syncLayoutDirectionWithLocale } from '@/utils/applyAppLocale'
...
const CustomDrawerContent = ({ closeDrawer }: CustomDrawerContentProps) => {
  const { loggedIn, language: authLanguage, refresh: refreshAuth } = useAuth()
  ...
  const updateLanguage = async (_language: string) => {
    try {
      const setLang = async (__language: string) => {
        ...
        await syncLayoutDirectionWithLocale(__language)
        await refreshAuth()
      }
